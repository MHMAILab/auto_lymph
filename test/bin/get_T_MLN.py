...
parser.add_argument('--wsi_path', default='./tiff/',
                    ...
                    help='Path to the input patient WSI file')
...
def run(patient_path):
    area_list = []
    pid = patient_path.split('/')[-1]
    vis_path = os.path.join(args.visualize_path, pid)
    save_path = os.path.join(args.mlna_path, pid)
    if not os.path.exists(vis_path):
        os.mkdir(vis_path)
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    for tif in os.listdir(patient_path):
        tif_path = os.path.join(patient_path, tif)
        print('generate img')
        ...
        print('compute area')
        areas = compute_area(lymph_mask, tumor_mask, img_ori, save_path)
        area_list.extend(areas)
    print('T/MLN:', np.mean(area_list))
