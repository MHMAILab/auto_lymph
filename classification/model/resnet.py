...
from classification.model.DropBlock import DropBlock2D
from classification.model.Scheduler import LinearScheduler
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, b_s=10, grid_size=9, drop_prob=[], block_size=7):
        ...
        self.layer1 = self._make_layer(
            block, 64, layers[0], stride=1, drop_prob=drop_prob[0], block_size=block_size)
        self.layer2 = self._make_layer(
            block, 128, layers[1], stride=2, drop_prob=drop_prob[1], block_size=block_size)
        self.layer3 = self._make_layer(
            block, 256, layers[2], stride=2, drop_prob=drop_prob[2], block_size=block_size)
        self.layer4 = self._make_layer(
            block, 512, layers[3], stride=2, drop_prob=drop_prob[3], block_size=block_size)
...
def resnet18(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load("/data/pre_ckpt/resnet18-5c106cde.pth")
...
def resnet34(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load("/data/pre_ckpt/resnet34-333f7ec4.pth")
        model_dict = model.state_dict()
        state_dict = {k: v for k, v in state_dict.items(
        ) if k in model_dict and "fc" not in k}
        model.load_state_dict(state_dict, False)
...
def resnet50(pretrained=True, **kwargs, ):
    ...
    if pretrained:
        ...
        state_dict = torch.load("/data/pre_ckpt/resnet50.pth")
        model_dict = model.state_dict()
        state_dict = {k: v for k, v in state_dict.items(
        ) if k in model_dict and "fc" not in k}
        model.load_state_dict(state_dict, False)
...
def resnet101(pretrained=True, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load("/data/pre_ckpt/resnet101-5d3b4d8f.pth")
        model_dict = model.state_dict()
        state_dict = {k: v for k, v in state_dict.items(
        ) if k in model_dict and "fc" not in k}
        model.load_state_dict(state_dict, False)
